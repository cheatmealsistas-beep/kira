...
import { Dumbbell, Target, AlertCircle, Clock, Check } from 'lucide-react';
    ...
    <Card>
      ...
      <CardContent className="space-y-6">
          ...
          <div className="flex gap-2 flex-wrap">
            {[2, 3, 4, 5, 6].map((days) => (
              <button
                ...
                className={cn(
                  'h-12 w-12 md:h-10 md:w-10 rounded-lg border text-sm font-medium transition-all touch-manipulation',
                  daysPerWeek === days
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            {experienceLevels.map((level) => (
              <button
                ...
                className={cn(
                  'rounded-lg border px-4 py-3 text-sm transition-all text-left touch-manipulation',
          ...
          <div className="grid grid-cols-2 sm:flex gap-2">
            {[30, 45, 60, 90].map((mins) => (
              <button
                ...
                className={cn(
                  'rounded-lg border px-4 py-3 text-sm transition-all touch-manipulation',
            ...
            {equipmentOptions.map((equipment) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    'flex items-center gap-1.5 rounded-full border px-4 py-2.5 text-sm transition-all touch-manipulation',
                    isSelected
                      ? 'border-rose-500 bg-rose-500 text-white'
                      : 'border-border hover:border-rose-300 hover:bg-rose-50 dark:hover:bg-rose-900/20'
                  )}
        ...
        <div className="space-y-3">
          ...
          <p className="text-sm text-muted-foreground">{t('limitationsHelp')}</p>
          <div className="flex flex-wrap gap-2">
          ...
          {selectedLimitations.length === 0 && (
            <p className="text-sm text-muted-foreground italic">{t('noLimitations')}</p>
          )}
        </div>
      </CardContent>
      <CardFooter>
